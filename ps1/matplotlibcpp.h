...
#else
  static std::string s_backend = "pdf";
#endif
...
inline bool annotate(std::string annotation, double x, double y, double u, double v)
{
    PyObject * xy = PyTuple_New(2);
    PyObject * uv = PyTuple_New(2);
    PyObject * str = PyString_FromString(annotation.c_str());

    PyTuple_SetItem(xy,0,PyFloat_FromDouble(x));
    PyTuple_SetItem(xy,1,PyFloat_FromDouble(y));
    PyTuple_SetItem(uv,0,PyFloat_FromDouble(u));
    PyTuple_SetItem(uv,1,PyFloat_FromDouble(v));

    PyObject* kwargs = PyDict_New();
    PyDict_SetItemString(kwargs, "xy", xy);
    PyDict_SetItemString(kwargs, "xytext", uv);

    PyObject* kwprops = PyDict_New();
    PyDict_SetItemString(kwprops, "arrowstyle", PyString_FromString("->"));
    PyDict_SetItemString(kwargs, "arrowprops", kwprops);
