...
public:
  ...
  void matvec_worker(const Vector& x, size_t begin, size_t end, Vector& y) const{
  ...
  void matvec(const Vector& x, Vector& y, size_t partitions) const{
    ...
    size_t                   col_blocksize = x.num_rows() / partitions;

    for (size_t i = 0; i < partitions; ++i) {
      threads.push_back(std::thread(&CSCMatrix::matvec_worker, this, std::cref(x), i*col_blocksize, (i+1)*col_blocksize, std::ref(y)));
    }
  ...
  void t_matvec_worker(const Vector& x, size_t begin, size_t end, Vector& y) const{
  ...
  void t_matvec(const Vector& x, Vector& y, size_t partitions) const{
    std::vector<std::thread> threads;
    size_t                   col_blocksize = num_cols() / partitions;

    for (size_t i = 0; i < partitions; ++i) {
      threads.push_back(std::thread(&CSCMatrix::t_matvec_worker, this, std::cref(x), i*col_blocksize, (i+1)*col_blocksize, std::ref(y)));
    }
