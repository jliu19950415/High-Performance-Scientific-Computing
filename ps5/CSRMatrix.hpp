...
public:
  ...
  void matvec(const Vector& x, Vector& y, size_t partitions) const{
    ...
    size_t                   row_blocksize = x.num_rows() / partitions;

    for (size_t i = 0; i < partitions; ++i) {
      threads.push_back(std::thread(&CSRMatrix::matvec_worker, this, std::cref(x), i*row_blocksize, (i+1)*row_blocksize, std::ref(y)));
    }
  ...
  void t_matvec(const Vector& x, Vector& y, size_t partitions) const{
    ...
    for (size_t i = 0; i < partitions; ++i) {
      threads.push_back(std::thread(&CSRMatrix::t_matvec_worker, this, std::cref(x), i*row_blocksize, (i+1)*row_blocksize, std::ref(y)));
    }
