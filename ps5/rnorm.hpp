...
double recursive_worker_a(const Vector& x, size_t begin, size_t end, size_t level) {
  ...
  size_t num_of_element = end - begin;
  ...
  if ((level == 0)){
    ...
    size_t mid = begin + (num_of_element / 2) ;
    return recursive_worker_a(std::ref(x), begin, mid, level-1) +  recursive_worker_a(std::ref(x) , mid, end, level-1);
  }
  return 0.0; 
}
...
double recursive_two_norm_a(const Vector& x, size_t levels) {
  std::future<double> sum = std::async(std::launch::async, recursive_worker_a, std::ref(x), 0, x.num_rows(), levels);

  return double(std::sqrt(sum.get()));
}
...
double recursive_worker_b(const Vector& x, size_t begin, size_t end, size_t level) {
  ...
  if ((level == 0)){
    ...
    size_t mid = begin + (num_of_element / 2) ;
    return recursive_worker_b(std::ref(x), begin, mid, level-1) +  recursive_worker_b(std::ref(x) , mid, end, level-1);
  }
  return 0.0; 
}
...
double recursive_two_norm_b(const Vector& x, size_t levels) {
  std::future<double> sum = std::async(std::launch::deferred, recursive_worker_b, std::ref(x), 0, x.num_rows(), levels);
