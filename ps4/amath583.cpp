        ...
        for (size_t i = ii; i < stop_i; i += 2) {
          for (size_t j = jj; j < stop_j; j += 2) {
            ...
            for (size_t k = kk; k < stop_k; ++k) {
              t00 += A(i, k) * temp(j, k);
              t01 += A(i, k) * temp(j + 1, k);
              t10 += A(i + 1, k) * temp(j, k);
              t11 += A(i + 1, k) * temp(j+1, k);
            }
...
void mult_trans_0(const Matrix& A, const Matrix& B, Matrix& C) {
  ...
  for (size_t i = 0; i < C.num_rows(); ++i) {
    for (size_t j = 0; j < C.num_cols(); ++j) {
      for (size_t k = 0; k < A.num_cols(); ++k) {
        //C(i, j) += A(i, k) * B(k, j); for multiplication
        C(i,j) += A(i,k) * B(j,k);
      }
    }
  }
...
void mult_ijk(const Matrix& A, const Matrix& B, Matrix& C) {
  ...
  // I'm confused about what's exactly asking for this question
  // Theoretically, since the A is accessed sequentially, the inner
  // loop advances the row number of B. If we take the transpose of 
  // B, we can access both matrices sequentially.

  // Matrix temp(B.num_cols(), B.num_rows());
  // for(size_t i = 0; i < temp.num_rows(); ++i)
  //   for(size_t j = 0; j < temp.num_cols(); ++j)
  //     temp(i,j) = B(j,i);
  ...
  // The code above is essentially the functions we wrote before
  Matrix temp = transpose(B);
  mult_trans_2(A,temp,C);
}
